...
vwap_sensitivity_long = 0.68  # vwap sensitivity
vwap_sensitivity_short = 0.90  # vwap sensitivity


def create_chart(symbol, interval, data):
    df = pd.DataFrame(data)
    df[[0,1,2,3,4,5]] = df[[0,1,2,3,4,5]].astype('float')
    df[0] = pd.to_datetime(df[0], unit='ms')
    df.index = df[0]

    vwap = ta.vwap(high=df[2], low=df[3], close=df[4], volume=df[5], length=6, anchor="min")
    ...
    d1 = (statistics.stdev([c1, c2, c3, c4, c5]) )*130*float(vwap_sensitivity_long)*1.2
    d2 = (statistics.stdev([c1, c2, c3, c4, c5]) )*130*float(vwap_sensitivity_short)*1.2
